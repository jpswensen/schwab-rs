...
Account summary balances include `true_cash_status` for SGOV sweep and funding decisions. `true_cash_status="verified"` means `true_cash` came from a margin-safe Schwab cash field: margin accounts use `initialBalances.totalCash` with `cashBalance` fallback, and cash accounts use `currentBalances.cashBalance` with `totalCash` fallback, then `initialBalances.cashBalance` if current balances are absent. `true_cash_status="unavailable"` means Schwab did not provide a usable true-cash source in the rendered balance data. `true_cash_status="not_applicable"` is reserved for account shapes where true cash does not apply. Do not treat `buying_power`, `available_funds`, `cash_available_for_trading`, `cash_available_for_withdrawal`, option buying power, or similar capacity fields as margin-safe cash.
