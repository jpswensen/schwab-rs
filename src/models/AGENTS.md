...
- CLI smoke tests in `tests/cli_smoke.rs` may assert serialized model-derived order JSON from dry-run commands; keep numeric output compatible with both default `Number` and `decimal` builds.
